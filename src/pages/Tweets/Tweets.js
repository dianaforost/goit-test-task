...
import { BtnToBack } from '../../components/BtnToBack/BtnToBack'
import { useLocation } from 'react-router-dom';
import { fetchUsers, followUser, fetchSortBy } from '../../components/api/api';
...
export default function Tweets() {
  ...
  const onFollow = async (id) => {
    const follows = await followUser(id);
    setFollow(follows);
  ...
  const handleFilterChange = async (option) => {
    ...
    const filteredData = await fetchSortBy(option);
    setFilteredUsers(filteredData);
  };
  const visibleUsers = filterOption === "all" ? users : filteredUsers;
