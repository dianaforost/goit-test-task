...
import { fetchUsers, followUser, fetchPaginationUsers } from '../../components/api/api';
...
export default function Tweets() {
    ...
    useEffect(() => {
        const fetchTrendingMovies = async () => {
            ...
            const user = await fetchPaginationUsers(currentPage, perPage);
            setUsers(user);
      ...
      const onFollow = async (id) => {
          const follows = await followUser(id, isFollowed);
          setFollow(follows);
          ...
          prevUsers.map((user) => {
            ...
            setIsFollowed(updatedUser.isFollowed);
            return updatedUser;
        }
        return user;
    })
...
return <div>
            ...
            <ul className={css.tweetsList}>
                {users === null ?(<h1>Loading</h1>) :(
                    <>{users.map(user => <li key={user.id} className={css.tweetsListItem}>
                        ...
                        {user.isFollowed ?(
                            <button className={css.activeClassName} type='button' onClick={() => onFollow(user.id)} disabled>Following</button>
